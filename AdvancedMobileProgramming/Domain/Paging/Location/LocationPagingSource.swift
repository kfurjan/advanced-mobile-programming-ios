...
final class LocationPagingSource: PagingSource {

    typealias Repo = LocationApiRepositoryImpl
    ...
    var restApi: Repo = LocationApiRepositoryImpl()
    ...
    ///
    /// - Returns: Returns list of ``Location`` objects, ``ApiError`` otherwise.
    ...
    private func onInitialLoad(page: Int) async throws -> [Response] {
        ...
        if daoData.count == 0 {
            ...
            dao.writeAll(objects: data.results.map { locationResult in
                LocationDao.toDaoObject(location: locationResult, info: data.info)
            })
            return dao.readAll().map(LocationDao.toGeneralObject)
        } else {
            return daoData.map(LocationDao.toGeneralObject)
        }
    }
